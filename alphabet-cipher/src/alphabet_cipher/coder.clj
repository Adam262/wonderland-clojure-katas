...
(defn encode [keyword message]
  (apply str
    (map encode-single-char (cycle keyword) message)))

(defn decode [keyword encrypted-message]
  (apply str
    (map decode-single-char (cycle keyword) encrypted-message)))
...
(defn decipher [encrypted-message message]
  (let [cycled (cycled-keyword message encrypted-message)]
    (subs cycled 0 (first
                     (filter
                       (fn [x] (= cycled (apply str (take (count cycled) (cycle (subs cycled 0 x))))))
                       (range (count cycled)))))))
